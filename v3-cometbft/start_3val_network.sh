...
## если все с нуля то удаляем 
##cd $BASE_DIR && rm -rf ./* && cd ../
##mkdir -p $BASE_DIR

# Или удалим только стейт (но не ключи!)
#echo " Node 1 - cleared only DATA dir... "
#cd $BASE_DIR/node1/data && rm -rf ./* && cd ../../ 

#echo " Node 2 - cleared only DATA dir... "
#cd $BASE_DIR/node2/data && rm -rf ./* && cd ../../ 

#echo " Node 3 - cleared only DATA dir... "
#cd $BASE_DIR/node3/data && rm -rf ./* && cd ../../ 

#echo " Node 4 - cleared only DATA dir... "
#cd $BASE_DIR/node4/data && rm -rf ./* && cd ../../ 

#echo " Node 5 - cleared only DATA dir... "
#cd $BASE_DIR/node5/data && rm -rf ./* && cd ../../ 


# --- ЭТАП 1: Инициализация и Копирование конфигов ---
echo "--- Инициализация $N_NODES нод (data state cleanup) ---"
...
for i in $(seq 1 $N_NODES); do
	...
	# 3. Патчим генезис 
	sed -i "s/4194304/44040192/g" $NODE_DIR/config/genesis.json  #Размер блока (21Mb - 22020096) 
	sed -i "s/10000000/-1/g" $NODE_DIR/config/genesis.json	 #gas limit 
